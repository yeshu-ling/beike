...
export default {
  ...
  methods:{
      ...
      returnhis(){
        ...
        localStorage.removeItem("xinlist")
         this.$router.push("/looksxin")
        //   window.location.reload();
      },
	  btn(){
		let xinlist = localStorage.getItem("xinlist");
		console.log("xinlist",xinlist);
		if(xinlist){
			if(this.flag){
				...
				localStorage.setItem("xinlist",JSON.stringify(this.detailmessagelist));
				window.location.reload();
	  ...
	  getpersonimg(){
		...
		this.personimg=localStorage.getItem("xinimg")
		this.userimg = JSON.parse(localStorage.getItem("users")).userimg;
		console.log("this.personimgWindow",this.personimg);
	  },
  },
  created(){
    localStorage.setItem("xinisreload",true);
    this.getpersonimg();
    // this.detailmessagelist = [
	// 		{
	// 			id:1,
	// 			img:localStorage.getItem("xinimg"),
	// 			messagedetail:"您好",
	// 			flagclass:true
	// 		}
	// 	];
	if(JSON.parse(localStorage.getItem("xinlist"))){
		this.detailmessagelist = JSON.parse(localStorage.getItem("xinlist"));
	}else{
		this.detailmessagelist = [
			{
				id:1,
